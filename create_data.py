...
def random_split(args):
    ...
    '''
    Triangle
    a = [284, 2157+1363,2157+2908+650]
    is_present = (single_gedi == a).all(axis=1).any()
    print(f"Is the target in the dataset? {is_present}") #belongs to single_gedi
    
    a = [127, 2157+1751,2157+2908+2514]
    is_present = (gene_tuples == a).all(axis=1).any()
    print(f"Is the target in the dataset? {is_present}") #belongs to gene_tuples, 
    HIV Infections (D015658) | Ritonavir (DB00352) | PTGS2 (5744)
    
    a = [75, 2157+743,2157+2908+866]
    is_present = (drug_tuples == a).all(axis=1).any()
    print(f"Is the target in the dataset? {is_present}") #belongs to drug_tuples
    
    a = [695, 2157+796,2157+2908+3785]
    is_present = (single_gedi == a).all(axis=1).any()
    print(f"Is the target in the dataset? {is_present}") #belongs to single_gedi
    
    drug_tuples
    a = [185, 2157+2721,2157+2908+3201]
    is_present = (single_gedi == a).all(axis=1).any()
    print(f"Is the target in the dataset? {is_present}") #belongs to single_gedi
    '''
