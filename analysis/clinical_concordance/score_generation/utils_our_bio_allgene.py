...
def get_binary_dataset(args, cycles, tuples, single):
    ...
    '''
    dise_def = torch.load(args.input_dir + 'dise_sim_pre20.pth')
    dise_fea = torch.load(args.input_dir + 'dise_sim_feat20.pth')
    dise_graph_def = torch.load(args.input_dir + 'dise_graph_pre20.pth')
    dise_graph_feat = torch.load(args.input_dir + 'dise_graph_feat20.pth')
    #dise_feat = torch.cat((dise_feat,dise_sim,dise_fea),1)
    '''

    drug_feat = torch.load(args.input_dir + 'drug_All.pth')
    '''
    drug_def = torch.load(args.input_dir + 'drug_sim_pre20.pth')
    drug_fea = torch.load(args.input_dir + 'drug_sim_feat20.pth')
    drug_graph_def = torch.load(args.input_dir + 'drug_graph_pre20.pth')
    drug_graph_feat = torch.load(args.input_dir + 'drug_graph_feat20.pth')
    #drug_feat = torch.cat((drug_feat,drug_sim,drug_fea),1)
    '''

    gene_feat = torch.load(args.input_dir + 'gene_All.pth')
    '''
    gene_def = torch.load(args.input_dir + 'gene_sim_pre100.pth')
    gene_fea = torch.load(args.input_dir + 'gene_sim_feat100.pth')
    gene_graph_def = torch.load(args.input_dir + 'gene_graph_pre100.pth')
    gene_graph_feat = torch.load(args.input_dir + 'gene_graph_feat100.pth')
    #gene_feat = torch.cat((gene_feat,gene_sim,gene_fea),1)
    '''
    ...
    if args.data_name == 'ms':
        #dise_feat = torch.cat((dise_feat, torch.zeros(args.dise_num, gene_feat_dim - dise_feat_dim)), dim=1) #ChemBERTa, MPNN, range, BERT_SMILES
        #drug_feat = torch.cat((drug_feat, torch.zeros(args.drug_num, gene_feat_dim - drug_feat_dim)), dim=1) #ChemBERTa, MPNN, range, BERT_SMILES
        # Robust zero-padding: bring every node-type feature up to the largest dim
        # (paper pads drug & gene up to the disease dim, e.g. 1792). Works regardless
        # of which feature file is largest.
        max_dim = max(dise_feat_dim, drug_feat_dim, gene_feat_dim)
        if dise_feat_dim < max_dim:
            dise_feat = torch.cat((dise_feat, torch.zeros(args.dise_num, max_dim - dise_feat_dim)), dim=1)
        if drug_feat_dim < max_dim:
            drug_feat = torch.cat((drug_feat, torch.zeros(args.drug_num, max_dim - drug_feat_dim)), dim=1)
        if gene_feat_dim < max_dim:
            gene_feat = torch.cat((gene_feat, torch.zeros(args.gene_num, max_dim - gene_feat_dim)), dim=1)

    data.x = torch.cat((dise_feat, drug_feat, gene_feat), dim=0) #save similarity matrices seperately

    '''
    # similarity matrices
    data.drug_def = drug_def
    data.drug_fea = drug_fea
    data.drug_graph_def = drug_graph_def
    data.drug_graph_feat = drug_graph_feat

    data.dise_def = dise_def
    data.dise_fea = dise_fea
    data.dise_graph_def = dise_graph_def
    data.dise_graph_feat = dise_graph_feat

    data.gene_def = gene_def
    data.gene_fea = gene_fea
    data.gene_graph_def = gene_graph_def
    data.gene_graph_feat = gene_graph_feat
    '''
...
def prepare_data(args):
    ...
    if args.task == 'multi-class':
        return split_motif_mc(args, cliques, tuples, single) #tuil split_motif_mc
