...
ANALYSIS_DIR = REPO_ROOT / "analysis" / "clinical_concordance"
...
def run_python(script: str, config: Path, extra: list[str] | None = None) -> None:
    command = [sys.executable, str(ANALYSIS_DIR / script), "--config", str(config)]
    if extra:
...
def run_figure5(config_path: Path) -> None:
    ...
    if rscript is None:
        raise RuntimeError(
            ...
            "patchwork packages, or rerun with --skip-plot."
        )
    ...
    render_expression = (
        "rmarkdown::render("
        f"input={r_string(ANALYSIS_DIR / 'plot_figure5.Rmd')},"
...
def main() -> None:
    ...
    parser.add_argument("--config", type=Path, default=ANALYSIS_DIR / "config.yaml")
    parser.add_argument(
        "--skip-build",
        action="store_true",
